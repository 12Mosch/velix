  ...
  at ["b"]`,
	...
	describe("decodeUnknownPromise / encodeUnknownPromise", () => {
		it("FiniteFromString", async () => {
			...
			const r1 = await decodeUnknownPromise("1").then(Result.succeed, (e) =>
				Result.fail(e.toString()),
			);
			deepStrictEqual(r1, Result.succeed(1));

			const r2 = await decodeUnknownPromise(null).then(Result.succeed, (e) =>
				Result.fail(e.toString()),
			);
			deepStrictEqual(r2, Result.fail("Expected string, got null"));

			const r3 = await encodeUnknownPromise(1).then(Result.succeed, (e) =>
				Result.fail(e.toString()),
			);
			deepStrictEqual(r3, Result.succeed("1"));

			const r4 = await encodeUnknownPromise(null).then(Result.succeed, (e) =>
				Result.fail(e.toString()),
			);
