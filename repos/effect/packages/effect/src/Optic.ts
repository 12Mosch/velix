...
export interface Optional<in out S, in out A> {
	...
	check<S, A>(
		this: Prism<S, A>,
		...checks: readonly [AST.Check<A>, ...Array<AST.Check<A>>]
	): Prism<S, A>;
	check<S, A>(
		this: Optional<S, A>,
		...checks: readonly [AST.Check<A>, ...Array<AST.Check<A>>]
 ...
 * @since 4.0.0
 */
export interface Traversal<in out S, in out A>
	extends Optional<S, ReadonlyArray<A>> {}
...
class OptionalImpl<S, A> implements Optional<S, A> {
	...
	refine<B extends A>(
		...
		return make(
			compose(
				this.node,
				new CheckNode([AST.makeFilterByGuard(refinement, annotations)]),
			),
		);
	}
	tag(tag: string): any {
...
function runRefinement<T extends E, E>(
	...
	return (e) =>
		AST.runChecks([AST.makeFilterByGuard(refinement, annotations)], e) as any;
}
