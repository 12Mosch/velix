...
export class Declaration extends Base {
	...
	) => Effect.Effect<any, Issue.Issue, any>;
...
export class Arrays extends Base {
	...
	getParser(recur: (ast: AST) => Parser.Parser): Parser.Parser {
		...
		function getParser(
			...
			index: number,
		): { readonly ast: AST; readonly parser: Parser.Parser } {
			if (index < elementLen) {
		...
		return Effect.fnUntracedEager(function* (oinput, options) {
			...
			if (ast.rest.length === 0 && len > elementLen) {
				for (let i = elementLen; i <= len - 1; i++) {
					const issue = new Issue.Pointer(
						[i],
						new Issue.UnexpectedKey(ast, input[i]),
					);
			...
			if (state.issues) {
				return yield* Effect.fail(
					new Issue.Composite(ast, oinput, state.issues),
				);
			}
...
>()({
	...
	step(s, _, exit, i) {
		if (exit._tag === "Failure") {
			...
			const issue = new Issue.Pointer(
				[i],
				new Issue.MissingKey(p.ast.context?.annotations),
			);
				...
				? iterateEager<
					...
					>()({
						onItem: Effect.fnUntracedEager(function* (s, [key, is]) {
							...
							) as Exit.Exit<Option.Option<PropertyKey>, Issue.Issue>;
						...
						step: (_s, _, exit: Exit.Exit<void, Issue.Issue>) =>
				...
				for (let i = 0; i < inputKeys.length; i++) {
					...
					if (!expectedKeysSet.has(key)) {
						...
						if (onExcessPropertyError) {
							const issue = new Issue.Pointer(
								[key],
								new Issue.UnexpectedKey(ast, input[key]),
							);
							if (errorsAllOption) {
								if (state.issues) {
...
>()({
	...
	step(s, p, exit) {
		if (exit._tag === "Failure") {
			...
			const issue = new Issue.Pointer(
				[p.name],
				new Issue.MissingKey(p.type.context?.annotations),
			);
			if (s.options.errors === "all") {
				if (s.issues) s.issues.push(issue);
				else s.issues = [issue];
		...
		return (oinput, options) => {
			...
			if (!eff) {
				return state.out
					...
					: Effect.fail(new Issue.AnyOf(ast, input, state.issues ?? []));
			}
...
export function makeFilter<T>(
	...
	return new Filter(
		(input, ast, options) =>
			Issue.make(input, ast, filter(input, ast, options)),
		annotations,
		aborted,
	);
}
...
export function makeFilterByGuard<T extends E, E>(
	...
	return new Filter(
		(input: E) =>
			is(input) ? undefined : new Issue.InvalidValue(Option.some(input)),
		annotations,
...
function fromConst<const T>(ast: AST, value: T): Parser.Parser {
...
const finiteToString = new Link(finiteString, Transformation.numberFromString);
...
const bigIntToString = new Link(bigIntString, Transformation.bigintFromString);
...
/** @internal */
export const unknownToJson = new Link(Json, Transformation.passthrough());
