	...
	const codec =
		from === to
			...
			: Schema.make<typeof schema>(
					AST.decodeTo(from, to, Transformation.passthrough()),
				);
				...
				if (ast.rest.length === 1) {
					propertySignatures.push(
						new AST.PropertySignature(
							REST_PROPERTY_NAME,
							new AST.Arrays(false, [], ast.rest),
						),
					);
				}
				return AST.decodeTo(
					recurOpenAI(
						new AST.Objects(propertySignatures, [], annotations, filters),
					),
					ast,
				...
				return AST.decodeTo(
					recurOpenAI(
						new AST.Arrays(
							false,
							[],
							[new AST.Arrays(false, [is.parameter, is.type], [])],
							annotations,
						),
					),
					ast,
...
function getFilters(
	checks: readonly [AST.Check<any>, ...AST.Check<any>[]],
	isArray: boolean,
): Array<Filter> {
