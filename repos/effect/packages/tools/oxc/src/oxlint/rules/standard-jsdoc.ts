 ...
 * @since 4.0.0
 */
export interface ParsedStandardJSDocFileDumpEntry
	extends ParsedStandardJSDocFile {
...
function getDumpState(cwd: string): StandardJSDocDumpState {
	...
	const global = globalThis as typeof globalThis & {
		[dumpStateKey]?: Map<string, StandardJSDocDumpState>;
	};
...
function registerDump(cwd: string, entry: ParsedStandardJSDocFileDumpEntry) {
	...
	process.once("exit", () => {
		...
		const files = [...state.entries].sort((a, b) =>
 ...
 * @since 4.0.0
 */
export function parseStandardJSDoc(
	raw: string,
): Result<ParsedCoreJSDoc, StandardJSDocParseError> {
...
function buildTags(
	...
	ParsedDeclarationTags | ParsedNamespaceTags | ParsedMemberTags,
	StandardJSDocParseError
> {
	const diagnostics: Array<StandardJSDocDiagnostic> = [];
	const allowed =
		scope === "declaration"
...
function getExport(
	...
	return (
		checker.getTypeOfSymbolAtLocation(target, location).getProperty(name) ??
...
function resolveJSDocLinkSymbol(
	...
	const lexical = checker.resolveName(
		root,
		sourceFile,
		ts.SymbolFlags.All,
		false,
	...
	const candidates = [
		...
		sourceModule === undefined
			? undefined
			: getExport(sourceModule, root, checker, sourceFile),
		entry.modulesByName?.get(root),
	];
	return candidates
		.map((candidate) =>
			resolveDottedSymbol(candidate, members, checker, sourceFile),
		)
...
export function parseStandardJSDocsFromESTree(input: {
	...
	const parseMembersFromFunction = (
	...
	const parseNestedMembersFromMember = (
		member: AstNode,
	): Array<ParsedMember> => [
	...
	const parseMembers = (
		members: ReadonlyArray<AstNode> | undefined,
	): Array<ParsedMember> => {
		...
		for (const member of members ?? []) {
			...
			const name = getRequiredNodeName(
				member,
				diagnostics,
				"Documented member",
			);
	...
	const parseMembersFromType = (
		type: AstNode | null | undefined,
	): Array<ParsedMember> => {
		if (!type) return [];
		switch (type.type) {
			...
			case "TSTypeReference":
				return (type.typeParameters?.params ?? []).flatMap(
					parseMembersFromType,
				);
			case "TSMappedType":
				return parseMembersFromType(type.typeAnnotation);
			case "TSIndexedAccessType":
				return [
					...parseMembersFromType(type.objectType),
					...parseMembersFromType(type.indexType),
				];
			case "TSFunctionType":
	...
	const parseNamespace = (
		...
		for (const statement of declaration.body?.body ?? []) {
			...
			const nestedDocumented = parseDocumented(
				statement,
				"namespace-declaration",
			);
...
function getStandaloneDeclarationBucket(
	declaration: AstNode,
...
function isDeclareNamespaceForSource(source: string, node: AstNode): boolean {
	return (
		node.declare === true ||
		/\bdeclare\s+namespace\b/.test(
			source.slice(node.range[0], Math.min(node.range[1], node.range[0] + 80)),
		) ||
		isInsideDeclareNamespaceForSource(source, node.range[0])
	);
}

function isInsideDeclareNamespaceForSource(
	source: string,
	index: number,
): boolean {
	const prefix = stripCommentsForNamespaceScan(source.slice(0, index));
	const matches = Array.from(
		prefix.matchAll(/\bdeclare\s+(?:namespace|module|global)\b/g),
	).reverse();
...
function getNodeName(node: AstNode): string {
	...
	if (node.type === "VariableDeclaration") {
		return ((node.declarations ?? []) as ReadonlyArray<AstNode>)
			.map(getNodeName)
...
function getRequiredNodeName(
	...
	diagnostics.push(
		diagnostic("missing-name", `${label} name could not be determined`),
	);
	return undefined;
}
...
function getRequiredExportedSpecifierName(
	...
	diagnostics.push(
		diagnostic("missing-name", "Export specifier name could not be determined"),
	);
	return undefined;
}
	...
	create(context) {
		...
		function validateLinks(node: AstNode, block: JSDocBlock) {
			...
			if (sourceFile === undefined) {
				if (!entry.reported) {
					...
					report(
						node,
						`Unable to validate JSDoc links: ${context.filename} is not included in ${tsconfigPath}`,
					);
				}
				return;
			}
			...
			for (const link of collectJSDocLinks(sourceFile, block)) {
				...
				if (urlRegex.test(target))
					...
					resolveJSDocLinkSymbol(link, target, sourceFile, checker, entry) ===
					undefined
				) {
		...
		function validateBlock(
			...
			if (block.parsed !== undefined) {
				const tags = buildTags(scope, block.parsed.tags);
				if (tags._tag === "Failure")
					reportDiagnostics(node, tags.error.diagnostics);
			}
		...
		function inspectParam(param: AstNode) {
			if (param.type === "TSParameterProperty")
				return inspectParam(param.parameter);
		...
		function inspectType(type: AstNode | null | undefined) {
			...
			switch (type.type) {
				...
				case "TSConditionalType":
					...
					break;
				case "TSTypeReference":
					for (const param of type.typeParameters?.params ?? [])
						inspectType(param);
					break;
				case "TSMappedType":
					inspectType(type.typeAnnotation);
		...
		function checkTypeLiteralMembers(
			members: ReadonlyArray<AstNode> | undefined,
		) {
		...
		function isAmbientModuleLike(node: AstNode): boolean {
			return (
				...
				!/\bnamespace\b/.test(
					source.slice(
						node.range[0],
						Math.min(node.range[1], node.range[0] + 80),
					),
				)
			);
		}
		...
		function checkNamespaceMemberExport(
			exportNode: AstNode,
			declaration: AstNode,
		) {
			if (declaration.type === "TSEnumDeclaration") {
			...
			if (declaration.type === "TSModuleDeclaration") {
				...
				if (block?.internal) return;
				if (requireBlock(exportNode, "namespace"))
					checkNamespaceMembers(declaration);
				return;
			}
			...
			const block = getLeadingBlock(exportNode);
			if (block?.internal) return;
			if (requireBlock(exportNode, "namespace-declaration"))
		...
		function checkExportedDeclaration(
			...
			if (checkedNamespaceMemberExports.has(key) || checkedExports.has(key))
				return;
			...
			const block = getLeadingBlock(exportNode);
			if (block?.internal) return;
			if (requireBlock(exportNode, "declaration"))
				checkDeclarationMembers(declaration);
		}

		function specifierBucket(
			exportNode: AstNode,
			specifier: AstNode,
		): ExportBucket {
			return exportNode.exportKind === "type" || specifier.exportKind === "type"
				? "type"
				: "value";
		}
		...
		return {
			...
			"Program:exit"() {
				...
				if (
					...
					(result.value.declarations.length > 0 ||
						result.value.namespaces.length > 0)
				) {
			...
			ExportNamedDeclaration(node: ESTree.Node) {
				...
				for (const specifier of exportNode.specifiers ?? [])
					checkExportSpecifier(exportNode, specifier);
			},
