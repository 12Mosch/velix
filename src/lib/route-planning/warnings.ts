...
export function getReadinessWarnings(
	route: PlannedRoute,
): Effect.Effect<RouteWarning[]> {
	return Effect.gen(function* () {
		const warnings = yield* getRouteWarnings(route);
		return warnings.filter((warning) => warning.category === "readiness");
	});
}
...
export function getProviderWarnings(
	...
	return Effect.succeed(getProviderWarningsSync(route));
}
...
export function buildRouteReadinessWarnings(
	...
	return Effect.gen(function* () {
		...
		const surfaceTotals = yield* getSurfaceDistanceTotals(route);
		const coarseShare =
		...
		if (route.windAnalysis) {
			const headwindShare =
				route.distanceMeters > 0
					? (route.windAnalysis.headwindDistanceMeters / route.distanceMeters) *
						100
					: 0;
			const averageHeadwind = Math.max(
				0,
				route.windAnalysis.averageHeadwindKmh,
			);
			...
			if (
				...
				warnings.push({
					...
					metricValue: formatWarningWindSpeed(
						route.windAnalysis.maxHeadwindKmh,
					),
				});
			} else if (
			...
			if (route.windAnalysis.maxCrosswindKmh >= 25) {
				warnings.push({
					...
					metricValue: formatWarningWindSpeed(
						route.windAnalysis.maxCrosswindKmh,
					),
				});
			}
		} else if (
		...
		const quality = yield* getRouteQuality(route);
