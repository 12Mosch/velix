import { Array as EffectArray, Effect, Option, Result, pipe } from "effect";
import type {
	Feature,
	FeatureCollection,
	LineString,
...
export function buildRouteGeoJson(
	route: PlannedRoute,
): Effect.Effect<FeatureCollection> {
	return Effect.sync(() => {
...
export function buildRouteSurfaceGeoJson(
	...
	return Effect.gen(function* () {
		const surfaceFeatures = yield* buildRouteSurfaceFeatures(
			route,
		...
		const features =
			surfaceFeatures.length > 0
				? surfaceFeatures
				: yield* buildRouteSurfaceFeatures(
						route,
...
export function buildRouteClimbGeoJson(
	...
	return Effect.gen(function* () {
		const profilePoints = yield* getRouteElevationAnalysisPoints(
			route.coordinates,
		);
...
export function buildRouteGradientGeoJson(
	route: PlannedRoute,
): Effect.Effect<FeatureCollection<LineString, RouteFeatureProperties>> {
	return Effect.gen(function* () {
		const sections = yield* getRouteGradientSections(route);
...
export function buildRouteTrafficStressGeoJson(
	...
	return Effect.gen(function* () {
		...
		for (const segment of yield* getRouteTrafficStressSegments(route)) {
